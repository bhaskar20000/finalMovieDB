...
const Popularcard = props => {
  ...
  return (
    <li className="li-home-element">
      <img className="li-image-element" src={imageUrl} alt="movieposterimage" />
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchDetailsHome = async () => {
      const urlhome = `https://api.themoviedb.org/3/movie/popular?api_key=883ef4828cba3f4d12947e02c71d11b3&language=en-US&page=${pageNumber}`
      try {
        ...
        if (response.ok) {
          ...
          updateMovieObj(prev => ({
            ...prev,
            responseStatus: responseList.isFailure,
          }))
        }
      } catch (e) {
